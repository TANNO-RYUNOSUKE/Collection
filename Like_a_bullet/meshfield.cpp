...
bool CMeshfield::MeshRay(const D3DXVECTOR3 & Start, const D3DXVECTOR3 & End)
{
	...
	{
		if (true == CManager::RayTri(Start, End, pVtx[m_nSplit_V + nPlus].pos, pVtx[0 + nPlus].pos, pVtx[m_nSplit_V + 1 + nPlus].pos))
		{
		...
		if (true == CManager::RayTri(Start, End, pVtx[1 + nPlus].pos, pVtx[m_nSplit_V + 1 + nPlus].pos, pVtx[0 + nPlus].pos))
		{
...
bool CMeshfield::Collision(D3DXVECTOR3 * ppos)
{
					...
					if ((vec02.z * vecP2.x) - (vec02.x * vecP2.z) > 0)
					{
						if ((vec03.z * vecP3.x) - (vec03.x * vecP3.z) > 0)
						{
							if (nor.y != 0.0f)
							{
								...
								if (dot > pos.y)
								{
									/*pDeb->Print(" pVtx[%d].pos,X:%f,Y:%f,Z:%f\n", m_nSplit_V + nPlus + nCnt, pVtx[m_nSplit_V + nPlus].pos.x, pVtx[m_nSplit_V + nPlus].pos.y, pVtx[m_nSplit_V + nPlus].pos.z);
									pDeb->Print("nor,X:%f,Y:%f,Z:%f\n", nor.x, nor.y, nor.z);
									pDeb->Print("↑起伏判定動作中 Dot:%f\n", dot);*/
									pos += nor * 20.0f;
					...
					if ((vec02.z * vecP2.x) - (vec02.x * vecP2.z) >= 0)
					{
						if ((vec03.z * vecP3.x) - (vec03.x * vecP3.z) >= 0)
						{
							if (nor.y != 0.0f)
							{
								...
								if (dot > pos.y)
								{
								/*	pDeb->Print(" pVtx[%d].pos,X:%f,Y:%f,Z:%f\n", 1 + nPlus + nCnt, pVtx[1 + nPlus].pos.x, pVtx[1 + nPlus].pos.y, pVtx[1 + nPlus].pos.z);
									pDeb->Print("nor,X:%f,Y:%f,Z:%f\n", nor.x, nor.y, nor.z);
									pDeb->Print("↓起伏判定動作中 Dot:%f\n", dot);*/
									*ppos = D3DXVECTOR3(pos.x, dot, pos.z);
									m_pVtxBuff->Unlock();
	...
	//for (int nCnt = 0; nCnt + nPlus < (m_nSplit_U * (m_nSplit_V));)
	//{
	//	D3DXVECTOR3 nor, vec1, vec2;
	//	vec1 = (pVtx[0 + nPlus].pos - pVtx[m_nSplit_V + nPlus].pos);
	//	vec2 = (pVtx[m_nSplit_V + 1 + nPlus].pos - pVtx[m_nSplit_V + nPlus].pos);
