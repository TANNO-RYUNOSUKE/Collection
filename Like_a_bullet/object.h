...
public:
	...
	//static CObject * GetObject(const int nPriority,const int nIdx) { return m_apObject[nPriority][nIdx]; }//オブジェクトの取得
	//static CObject ** GetpObject(const int nPriority, const int nIdx) { return &(m_apObject[nPriority][nIdx]); }//オブジェクトの取得

	virtual void Set(D3DXVECTOR3 pos, D3DXVECTOR3 rot, float fHeight, float fWidth) {};
