...
//当たり判定
//=============================================
bool CObjectX::CorrectMovementForBoundingBox(D3DXVECTOR3 prevPosition, D3DXVECTOR3* pCurrentPosition, D3DXVECTOR3* pMovement)
{
...
void CDebri::Update()
{
	...
	if (m_nLife <= 0)
	{
		CAfterImage::Create(GetName(), GetPos(), GetRot(), D3DXCOLOR(0.9f, 0.9f, 1.0f, 0.8f), 60);
		Release();
	}
}
void CDebri::Draw()
...
HRESULT CAfterImage::Init()
{
	m_pMat = {};
	CObjectX::Init();
	return S_OK;
}
void CAfterImage::Uninit()
{
	CObjectX::Uninit();
}
...
void CAfterImage::Draw()
{
	...
	pDevice->SetRenderState(D3DRS_LIGHTING, TRUE); // ライティングを再度有効にする
...
CAfterImage * CAfterImage::Create(std::string pName, D3DXVECTOR3 pos, D3DXVECTOR3 rot, D3DXCOLOR col, int nLife, int nPriority )
{
	CAfterImage * pObject3D = NULL;

	pObject3D = DBG_NEW CAfterImage(nPriority);
	pObject3D->SetName(pName);
	pObject3D->SetPos(pos);
	pObject3D->SetRot(rot);
	pObject3D->m_col = col;
...
//色変更
//=============================================
void CAfterImage::SetColor(D3DXCOLOR col)
{
	CXFile* pXFile = CManager::GetXFiles();
	D3DXMATERIAL* pMat = NULL; // マテリアルデータへのポインタ
	...
	{
		// 新しいマテリアルをコピーして変更を適用
		D3DMATERIAL9 newMat = pMat[nCntMat].MatD3D;
		newMat.Diffuse = col;
		newMat.Ambient = col;
		newMat.Emissive = col;
