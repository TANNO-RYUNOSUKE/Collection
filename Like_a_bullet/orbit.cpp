...
void COrbit::Update(void)
{
	...
	if (m_bDisp == true)
	{
		...
		for (int nCnt = 0; nCnt < m_nLength * 2 + 1; nCnt += 2)
		{
			...
			{
				pVtx[nCnt].pos = m_pPosOld[nCnt - 2] +((m_pPosOld[nCnt - 1] - m_pPosOld[nCnt - 2]) /(float)m_nLength / (float)2);
				pVtx[nCnt + 1].pos = m_pPosOld[nCnt - 1] + ((m_pPosOld[nCnt - 2] - m_pPosOld[nCnt - 1]) / (float)m_nLength / (float)2);
			}
		}
